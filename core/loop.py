...
from modules.tools import summarize_tools, clean_text
import re
        ...
        for step in range(max_steps):
            ...
            while lifelines_left >= 0:
                ...
                self.context.memory.add_tool_output(
                    ...
                    tool_args={"perception": perception},
                    tool_result={"result": perception},
                    success=True,
                    tags=["perception"]
                )
                ...
                if re.search(r"^\s*(async\s+)?def\s+solve\s*\(", plan, re.MULTILINE):
                    ...
                    print(f"memory: {self.context.memory.get_session_items()}")
...
class ModelManager:
    def add_tool_output(self, tool_name, tool_output, tool_args=None):
        """Add tool output to memory"""
        # In the run method
        self.context.memory.add_tool_output(
            tool_name=tool_name,
            tool_output=result,
            tool_args=tool_args  # Now this parameter will be accepted
        )
